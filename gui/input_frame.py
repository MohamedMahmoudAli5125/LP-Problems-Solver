...
class InputFrame(ctk.CTkScrollableFrame):
    def __init__(self, master, output_ref, **kwargs):
        ...
        self.label = ctk.CTkLabel(self, text="Enter the number of variables:")
        self.label.pack(pady=(10, 5))
        
        self.entry = ctk.CTkEntry(self, placeholder_text="e.g., 3")
        self.entry.pack(pady=5)
        
        self.button = ctk.CTkButton(self, text="Generate Objective Row", command=self.get_input)
        ...
        self.constraints_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.constraints_frame.pack(pady=10, fill="both", expand=True)
         
        self.addConstraints_button = ctk.CTkButton(self, text="Add Constraints", command=self.add_constraint_row, state="disabled", width=120)        
    ...
    def get_input(self):
        try:
            ...
            if self.num_vars <= 0: raise ValueError
            ...
            for child in self.constraints_frame.winfo_children(): child.destroy()
            self.all_constraints_widgets = []
        except ValueError:
            self.status_label.configure(text="Please enter a valid positive integer.", text_color="red")

    def create_bounds_section(self):
        for child in self.bounds_frame.winfo_children(): child.destroy()
        self.var_type_widgets = []
